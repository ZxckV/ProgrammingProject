...
function love.update(dt)
	...
	if love.keyboard.isDown('space') then -- If the space bar is pressed
		if state == 'menu' then -- If the game is in the menu state
			...
			changeState('info') -- Change the state to info
		end
	end
	...
	if love.keyboard.isDown('f') then -- If the F key is pressed
		if state == 'play' then -- If the game is in the play state
			if complete == true then -- If complete is true
				...
				sounds.press:play() -- Play the press sound
				changeState('end') -- Change the state to end
			end
		end
	end
	...
	if love.keyboard.isDown('q') then -- If the Q key is pressed
		if state == 'pause' then -- If the game is in the pause state
			...
			sounds.press:play() -- Play the press sound
			gameReset()
			changeState('menu') -- Change the state to menu
		end
	end
	...
	if love.keyboard.isDown('1') then -- If the 1 key is pressed
		if state == 'info' then -- If the game is in the
			page = 1
		end
	end

	if love.keyboard.isDown('2') then -- If the 1 key is pressed
		if state == 'info' then -- If the game is in 
			page = 2 
		end
	end
...
function love.draw()
	...
	if state == 'loading' then -- If the game is in the loading state
		...
		love.graphics.rectangle('line', (((window_w / 2) - (rec_w / 2)) - 4), (((window_h / 2) - (rec_h / 2)) - 4), (rec_w + 8), (rec_h + 8))
		love.graphics.setColor(1, 1, 1, 1)
		love.graphics.rectangle('fill', ((window_w / 2) - (rec_w / 2)), ((window_h / 2) - (rec_h / 2)), percent, rec_h)
	...
	if state == 'info' then
		...
		if page == 1 then
			...
			love.graphics.setColor(1, 1, 1, 1)
			love.graphics.printf('Explore the map', 0, 200, window_w, 'center')
			love.graphics.printf('Collect the crosses by walking up to them', 0, 300, window_w, 'center')	
			love.graphics.printf('Use the pause menu to see how many you have', 0, 400, window_w, 'center')	
	
			love.graphics.printf('[2] Next page', 0, window_h - 50, window_w, 'center')
		...
		if page == 2 then
			...
			love.graphics.setBackgroundColor(0.251, 0.627, 1, 1)
			love.graphics.setColor(1, 1, 1, 1)
			love.graphics.printf('WARNING: Once you return to the menu, all progress will be lost', 0, 200, window_w, 'center')
			love.graphics.printf('You cannot save at any point', 0, 300, window_w, 'center')	
			love.graphics.printf('Press [ENTER] to continue', 0, 400, window_w, 'center')		

			love.graphics.printf('[1] Previous page', 0, window_h - 50, window_w, 'center')
...
function gameReset()
	...
	-- Resetting all game progression
	player.x = 2704
	player.y = 594
